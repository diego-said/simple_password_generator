...
public class PasswordsDbHelper extends DatabseHelper {
	...
	public List<PasswordSettings> getListPasswordSettings() {
		...
		try {
			...
			query.append("select * from " + TABLE_PASSWORD_SETTINGS.TABLE_NAME);
			query.append(" a left join " + TABLE_PASSWORDS.TABLE_NAME + " b on a.");
			query.append(TABLE_PASSWORD_SETTINGS.COLUMN_ID + " = b." + TABLE_PASSWORDS.COLUMN_PASSWORD_SETTINGS_ID);
			query.append(" order by a." + TABLE_PASSWORD_SETTINGS.COLUMN_NAME);
			query.append(" , b." + TABLE_PASSWORDS.COLUMN_ID);
